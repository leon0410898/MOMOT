...
def sigmoid_focal_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2, mean_in_dim1=True, **kwargs):
    """
    ...
    if alpha >= 0:
        ...
        loss = alpha_t * loss
    if 'disappear_mask' in kwargs:
        loss = loss * kwargs['disappear_mask']
    if mean_in_dim1:
        if loss.shape[1] == 0:
            return torch.tensor(0, dtype=torch.float32, device=loss.device)
