...
class Detector(object):
    ...
    def visualize_img_with_bbox(img_path, img, boxes=None, identities=None):
        if boxes is not None:
            img_show = draw_bboxes(img, boxes, identities=identities)
    ...
    def detect(self, prob_threshold=0.7, area_threshold=100, vis=False):
        ...
        for i in tqdm(range(0, self.img_len)):
            ...
            frame_res = self.model(data)
            boxes_xyxy = box_cxcywh_to_xyxy(torch.cat([frame_res['pred_det_boxes'][0], 
                                                       frame_res['pred_track_boxes'][0]]))\
                                          * torch.tensor([seq_w, seq_h, seq_w, seq_h], device=frame_res['pred_det_boxes'][0].device)
            identities = torch.cat([frame_res['det_gt_idx'][0],
                                 frame_res['track_gt_idx'][0]])
...
if __name__ == '__main__':
    ...
    #seq_nums = os.listdir(os.path.join(args.mot_path))
