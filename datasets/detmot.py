...
class DetMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, transforms):
        ...
        self.visibilty_thresh = args.visibility_thresh
        self.vis = args.vis
        self.video_dict = {}
    ...
    def _targets_to_instances(targets: dict, img_shape) -> Instances:
        ...
        gt_instances.visibility = targets['visibility']
        return gt_instances
    ...
    def filter_invisible_obj(self, gt_instances):
        for frame_id, gt in enumerate(gt_instances):
            if frame_id == 0:
                visible_gt_ids = gt[gt.visibility > self.visibilty_thresh].obj_ids.tolist()
                visible_ids = (gt.visibility > self.visibilty_thresh).nonzero(as_tuple=True)[0]
            else:
                visible_ids = []
                for id, gt_id in enumerate(gt.obj_ids):
                    if gt_id not in visible_gt_ids:
                        if gt.visibility[id] > self.visibilty_thresh:
                            visible_gt_ids.append(gt_id.item())
                    else:
                        visible_ids.append(id)
            gt_instances[frame_id]  = gt[visible_ids]
        return gt_instances
    ...
    def __getitem__(self, idx):
        ...
        gt_instances = self.filter_invisible_obj(gt_instances)
