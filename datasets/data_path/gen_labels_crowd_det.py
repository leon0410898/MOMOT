...
def gen_labels_crowd(data_root, label_root, ann_root):
    ...
    for i, ann_data in enumerate(anns_data):
        ...
        for i in range(len(anns)):
            ...
            x += w / 2
            y += h / 2
            label_fpath = os.path.join(label_root, img_path.split('/')[-1].replace('.png', '.txt').replace('.jpg', '.txt'))
...
if __name__ == '__main__':
    data_val = '/datasets/crowdHuman/images'
    label_val = '/datasets/crowdHuman/labels_with_ids/val'
    ann_val = '/datasets/crowdHuman/annotation_val.odgt'
    data_train = '/datasets/crowdHuman/images'
    label_train = '/datasets/crowdHuman/labels_with_ids/train'
