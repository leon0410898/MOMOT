...
class DetMOTDetection:
    ...
    @staticmethod
    def _targets_to_instances(targets: dict, img_shape, max_inst_count=300) -> Instances:
        gt_instances = Instances(tuple(img_shape))
        gt_instances.boxes = targets['boxes'][:max_inst_count]
        gt_instances.labels = targets['labels'][:max_inst_count]
        gt_instances.obj_ids = targets['obj_ids'][:max_inst_count]
        gt_instances.area = targets['area'][:max_inst_count]
        return gt_instances
    ...
    def __getitem__(self, idx):
        ...
        for img_i, targets_i in zip(images, targets):
            gt_instances_i = self._targets_to_instances(targets_i, img_i.shape[1:3], max_inst_count=self.args.num_queries)
