...
class Detector(object):
    ...
    @staticmethod
    def visualize_img_with_bbox(img_path, img, boxes=None, identities=None):
        if boxes is not None:
            img_show = draw_bboxes(img.numpy(), boxes, identities=identities)
        cv2.imwrite(img_path, img_show)

    def detect(self, prob_threshold=0.7, area_threshold=100, vis=False):
        debug = Debug()
        debug.register_hooks(self.model)
        ...
        for i, (cur_img, ori_img) in enumerate(tqdm(loader)):
            ...
            data = {}
            data['imgs'] = cur_img.cuda().float()

            outputs = self.model(data)
            
            if 'pred_track_boxes' in outputs:
                boxes_xyxy = box_cxcywh_to_xyxy(torch.cat([outputs['pred_track_boxes'][0], 
                                                           outputs['pred_det_boxes'][0]]))\
                                            * torch.tensor([seq_w, seq_h, seq_w, seq_h], device=outputs['pred_det_boxes'][0].device)
                identities = torch.cat([outputs['track_gt_idx'][0],
                                        outputs['det_gt_idx'][0]])
                self.write_results(txt_path=os.path.join(self.predict_path, f'{self.seq_num}.txt'),
                                frame_id=(i + 1),
                                bbox_xyxy=boxes_xyxy,
                                identities=identities)
            else:
                det_scores = outputs['pred_det_logits'][0, :, 0].sigmoid() #(300,)
                det_src_ids = (det_scores >= 0.7).nonzero(as_tuple=True)[0]
                boxes = outputs['pred_det_boxes'][0, det_src_ids, :]
                boxes_xyxy = box_cxcywh_to_xyxy(boxes) * torch.tensor([seq_w, seq_h, seq_w, seq_h], device=boxes.device)
                identities = torch.zeros(det_src_ids.shape, device=boxes.device)

            vis=True
            if vis:
                #debug.viz_attn(ori_img, boxes=boxes_xyxy, identities=identities, frame_id=counter)
                cur_vis_img_path = os.path.join(self.save_path, 'frame_{}.jpg'.format(i))
                self.visualize_img_with_bbox(cur_vis_img_path, ori_img,  boxes=boxes_xyxy, identities=identities)

        self.model.track_instances = None
        #filter_pub_det(os.path.join(self.predict_path, f'{self.seq_num}.txt'),
        #                f'./MOT17/images/test/{self.seq_num}/det/det.txt')
...
if __name__ == '__main__':
    ...
    seq_nums = os.listdir('./MOT17/images/train')
    accs = []
    seqs = []
    for seq_num in seq_nums:
